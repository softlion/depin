...
"@
  ...
  if(!$Debug) {
    $all = iwr 'https://raw.githubusercontent.com/softlion/depin/main/honeygain/honeygain.sh' -UseBasicParsing | % Content
  } else {
    $all = Get-Content -Raw -Path (Join-Path $PSScriptRoot "honeygain.sh")
  }

  if (![string]::IsNullOrEmpty($Email)) {
    $all = "ACCOUNT_EMAIL=$Email; `n" + $all
  }
  if (![string]::IsNullOrEmpty($Password)) {
    $all = "ACCOUNT_PASSWORD=$Password; `n" + $all
  }
  if (![string]::IsNullOrEmpty($DeviceName)) {
    $all = "DEVICE_NAME=$DeviceName; `n" + $all
  }
