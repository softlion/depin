...
"@
  ...
  if(!$Debug) {
    $all = iwr 'https://raw.githubusercontent.com/softlion/depin/main/presearch/presearch.sh' -UseBasicParsing | % Content
  } else {
    $all = Get-Content -Raw -Path (Join-Path $PSScriptRoot "presearch.sh")
  }

  if (![string]::IsNullOrEmpty($RegistrationCode)) {
    $all = "REGISTRATION_CODE=$RegistrationCode; `n" + $all
  }
