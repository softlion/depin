...
"@
  ...
  if(!$Debug) {
    $all = iwr 'https://raw.githubusercontent.com/softlion/depin/main/grass/grass.sh' -UseBasicParsing | % Content
  } else {
    $all = Get-Content -Raw -Path (Join-Path $PSScriptRoot "grass.sh")
  }

  if (![string]::IsNullOrEmpty($UserIDs)) {
    $all = "userIds=$UserIDs; `n" + $all
  }
